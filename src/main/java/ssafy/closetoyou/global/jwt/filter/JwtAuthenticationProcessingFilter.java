package ssafy.closetoyou.global.jwt.filter;
...
import ssafy.closetoyou.global.jwt.util.PasswordUtil;
...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
    ...
    public void checkRefreshTokenAndReIssueAccessToken(HttpServletResponse response, String refreshToken) {
        refreshTokenRepository.findUserIdByRefreshToken(refreshToken)
                .ifPresent(id -> {
                    ...
                    String reIssuedAccessToken = jwtService.createAccessToken(user.getEmail(),user.getId());
    ...
    private String reIssueRefreshToken(User user) {
        String reIssuedRefreshToken = jwtService.createRefreshToken(user.getEmail(),user.getId());
        RefreshToken userRefreshToken = RefreshToken.builder().userId(user.getId()).refreshToken(reIssuedRefreshToken).build();
     ...
     * 유효한 토큰이면, 액세스 토큰에서 extractUserId로 ID를 추출한 후 findOne()로 해당 Id를 사용하는 유저 객체 반환
    ...
    public void checkAccessTokenAndAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        if (accessTokenOptional.isPresent()) {
            ...
            if (jwtService.isTokenValid(accessToken)) {
                Optional<String> emailOptional2 = jwtService.extractEmail(accessToken);
    ...
    public void saveAuthentication(User myUser) {
        String password = myUser.getPassphrase();
